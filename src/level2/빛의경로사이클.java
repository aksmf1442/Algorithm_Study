 ...
 * <p>
 * 제한사항 1. ≤ grid의 길이 ≤ 500 2. ≤ grid의 각 문자열의 길이 ≤ 500 3. grid의 모든 문자열의 길이는 서로 같습니다. 4. grid의 모든
 * 문자열은 'L', 'R', 'S'로 이루어져 있습니다.
 * <p>
 * 알아야 할 것 1. 빛이 "S"가 써진 칸에 도달한 경우, 직진합니다. 2. 빛이 "L"이 써진 칸에 도달한 경우, 좌회전을 합니다. 3. 빛이 "R"이 써진 칸에 도달한
 * 경우, 우회전을 합니다. 4. 빛이 격자의 끝을 넘어갈 경우, 반대쪽 끝으로 다시 돌아옵니다. 예를 들어, 빛이 1행에서 행이 줄어드는 방향으로 이동할 경우, 같은 열의
 * 반대쪽 끝 행으로 다시 돌아옵니다.
 * <p>
 * 문제 쉽게 이해하기 1. 하나의 노드는 LEFT, RIGHT, UP, DOWN의 출발 가능한 위치가 있다. 2. 한 곳을 정하고 순환을 시작하는데 한 번 더 나가야 할 경우가
 * 생긴다면 그 곳의 사이클은 끝났다고 생각하면 된다. 3. 예시와 같이 1행1열에서 밑으로 하자.
 */
