...
/**
 * 1. 뒤집어야 하는 카드의 수를 모두 구한다.
 * 2. 카드를 뒤집는 순서의 경우의 수를 모두 구한다.(dfs로 모든 경우의 수 찾기)
 * 3. 짝을 뒤집을 때 어떤 것을 먼저 뒤집을 지에 대한 경우의 수도 고려한다.
 * -> 1과 1을 뒤집는다고 가정했을 때 첫 번째 1을 먼저 뒤집을 것인지 뒤의 1을 먼저 뒤집을 것인지 두 경우의 수를 모두 구하기.
 * 4. 해당 행, 열로 이동하는 방법 중에 가장 빠른 방법을 찾는다.(bfs로 최소 이동거리 찾기)
 */
